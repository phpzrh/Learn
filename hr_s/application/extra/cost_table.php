...
//处理PKG段的prdno
function get_pkg_partno($data){
	foreach($data as $k => $v){
		$partno_p = $v['partno'];
		//去除Pkg段
		$partno_p = str_ireplace('_PKG','',$partno_p);
		$partno_p_tmp = explode('-',$partno_p);
		$partno_p = $partno_p_tmp[0];
		Db::name('cost_bom')->where('id',$v['id'])->setField('partno_p',$partno_p);
	}
}

//处理FT段的prdno
function get_ft_partno($data){
	foreach($data as $k => $v){
		$partno_p = $v['partno'];
		$partno_p_tmp = explode('-',$partno_p);
		$partno_p = $partno_p_tmp[0];
