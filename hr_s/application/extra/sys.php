...
function get_user_work_time_more($user_gh,$date,$site_id=0){
	...
	$is_card_early=db('entry_dt_sh')->where("emp_no='".$user_gh."' and ctrl_id in (2,23) and status=1 and entry_dt between '".$date.' '.config('hr_begin_time')."' and '".$date." 07:29:59'")->order('entry_dt')->select();
	...
	if(count($is_card_early)>0){
		$card_arr_temp=db('entry_dt_sh')->where("emp_no='".$user_gh."' and ctrl_id in (2,23) and status=1 and entry_dt between '".$date." ".$hr_begin_str."' and '".$date." 23:59:59'")->order('entry_dt')->select();
		$card_arr=array();
		...
		$card_arr=db('entry_dt_sh')->where("emp_no='".$user_gh."' and ctrl_id in (2,23) and status=1 and entry_dt between '".$date.' '.config('hr_begin_time')."' and '".$date." 23:59:59'")->group('entry_dt')->order('entry_dt')->select();
	}
		...
		if($site_id !=7){
			...
			if($row_info['last_annual_num']-$row_info['local_note_hour']>0){
				...
				if(($row_info['last_annual_num']+$row_info['last_repair_num'])-$row_info['local_note_hour']>0){
					...
					//上月结算年休+补休 不够本月申请休假扣 ,多余部分结算到事假
					$sql="update sw_hr_cal set local_annual_num=0,local_repair_num=0,local_num=0,casual_leave=ifnull(".((($row_info['last_annual_num']+$row_info['last_repair_num']))-$row_info['local_note_hour']).",0) where id=".$id;
					db()->query($sql);
				}
			}
		}else{
...
function get_user_month_hr($date,$user_id){
	...
	if(count($user_hr)>0){
		...
		$note_sql="SELECT note_check_status,note_step,user_id,begin_time,end_time FROM sw_user_note where user_id='$user_id' and (begin_time>='$month_arr[0]' and end_time<='$month_arr[1]' or end_time>'".date("Y-m-d",strtotime($month_arr[0] . "-1 day"))."')";
 ...
 * 规避单位时间内多次打卡情况
 */
function set_entry_status($month_range=array()){
	//header("Content-type: text/html; charset=utf-8");
	...
	//将某时间段中设置为无效的记录设置为有效
	$sql="update sw_entry_dt_sh set status=1 where entry_date between '".$month_range[0]." 00:00:00' and '".$month_range[1]." 23:59:59'";
	db()->query($sql);
	...
	foreach ($entry_arr as $key=>$val){
		//获取当前用户某日多次打卡时间数
		$sql="select * from sw_entry_dt_sh where emp_no='".$val['emp_no']."' and entry_dt between '".$val['entry_date']." 00:00:00' and '".$val['entry_date']." 23:59:59' and ctrl_id in (23,2) ;";
			...
			if($k>0){
				...
				if(date('H',strtotime($v['entry_dt']))<=12){
					...
					if(strtotime($v['entry_dt'])<=($temp_time+$sec_num)){
						db('entry_dt_sh')->where('id='.$v['id'])->setField('status',0);
					...
					if(strtotime($v['entry_dt'])>($temp_time+$sec_num)){
						...
						db('entry_dt_sh')->where('id='.$temp_arr['id'])->setField('status',0);
