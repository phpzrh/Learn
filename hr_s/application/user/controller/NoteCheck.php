		...
		switch ($note_info['note_step']){
			case 1:
				if($data['check_val']==0){
					...
					$update_arr['note_log']=$note_info['note_log'].'<br />['.date('Y-m-d H:i:s',time()).'] 代理人'.get_user_nickname()."审核不通过,申请单流程结束!<br />";
					...
					$update_arr['note_log']= $note_info['note_log'].'<br />['.date('Y-m-d H:i:s',time()).'] 代理人'.get_user_nickname()."审核通过,下一个流程,考勤主管审核!<br />";
			...
			case 2:
				if($data['check_val']==0){
					...
					$update_arr['cur_user_id']=0;
					$update_arr['note_check_status']=0;
					$update_arr['note_log']=$note_info['note_log'].'<br />['.date('Y-m-d H:i:s',time()).'] 考勤主管'.get_user_nickname()."审核不通过,申请单流程结束!<br />";
				}else{
					...
					if($note_info['note_type']==3){
						...
						$update_arr['note_log']=$note_info['note_log'].'<br />['.date('Y-m-d H:i:s',time()).'] 考勤主管'.get_user_nickname()."审核通过,意见:".$data['remark'].",申请单生效,流程结束!<br />";
						...
						if($note_info['note_hour']<=config('hr_holiday_adv_manage_val')*8){
							...
							$update_arr['note_log']=$note_info['note_log'].'<br />['.date('Y-m-d H:i:s',time()).'] 考勤主管'.get_user_nickname()."审核通过,申请单生效,流程结束!<br />";						
						}else{
							$update_arr['note_step']=3;
							$update_arr['hr_check_status']=1;
							$update_arr['hr_check_remark']=$data['remark'];
							$update_arr['cur_user_id']=$note_info['hr_adv_user_id'];
							$update_arr['note_log']=$note_info['note_log'].'<br />['.date('Y-m-d H:i:s',time()).'] 考勤主管'.get_user_nickname()."审核通过,下一个流程,考勤上级主管审核!<br />";
						}
					}
			...
			case 3:
				if($data['check_val']==0){
					...
					$update_arr['cur_user_id']=0;
					$update_arr['note_check_status']=0;
					$update_arr['note_log']=$note_info['note_log'].'<br />['.date('Y-m-d H:i:s',time()).'] 考勤上级主管'.get_user_nickname()."审核不通过,申请单流程结束!<br />";
				}else{
					$update_arr['note_step']=4;
					...
					//拆单
					$this->split_note($note_info['id']);					
					$update_arr['note_log']=$note_info['note_log'].'<br />['.date('Y-m-d H:i:s',time()).'] 考勤上级主管'.get_user_nickname()."审核通过,申请单生效,流程结束!<br />";					
				}
		}
	...
	function split_note($id){
		...
		if($note_info['note_hour']<=8 && (date('Y-m-d',strtotime($note_info['begin_time'])) == date('Y-m-d',strtotime($note_info['end_time']))) ){
			...
			*/
			...
			if($user_work_time<$user_need_work_time){
				//请假第一日有工作时间,需补时差,最多补8小时时差避免第一天扣9小时情况
				if(($user_need_work_time-$user_work_time)>=8){
					if(($user_need_work_time-$user_work_time)==8){
						//当天工作了1小时
						$note_item_arr['note_hour']=7;
					}else{
						$note_item_arr['note_hour']=8;
					}					
				}else{
