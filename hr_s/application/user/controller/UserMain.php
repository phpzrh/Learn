			...
			switch ($data['note_type']){
				case 1:
					...
					*/
					...
					//开始时间范围判断
					$is_hava_note=db('user_note')->where('user_id='.$user_id." and '".$data['begin_time']."' between date_add(begin_time,interval 1 SECOND) and date_sub(end_time,interval 1 SECOND) and status=1 and note_type=1")->find();
					...
					//结束时间短判断
					$is_hava_note=db('user_note')->where('user_id='.$user_id." and '".$data['end_time']."' between date_add(begin_time,interval 1 SECOND) and date_sub(end_time,interval 1 SECOND) and status=1 and note_type=1")->find();
					if($is_hava_note){
						$insert_flag=false;
						$msg="申请单日期范围内有重复申请单,ID为".$is_hava_note['id'].",请核对申请单日期范围!";
						break;
					...
					//时间完全相等判断
					$is_hava_note=db('user_note')->where('user_id='.$user_id." and begin_time='".$data['begin_time']."' and end_time='".$data['end_time']."' and status=1 and note_type=1")->find();
					if($is_hava_note){
						$insert_flag=false;
						$msg="申请单日期范围内有重复申请单,ID为".$is_hava_note['id'].",请核对申请单日期范围!";
						break;
