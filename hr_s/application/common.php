...
function make_Nav($data,$id_Group = array(),$parameter = '',$is_float = '0'){
	...
	foreach($data as $k => $v){
		...
		$tree .= '<input '.$check.'style="margin-left:3px;" type="checkbox" name="pri_id" value="'.$v['id'].'"><a nodeid="'.$v['id'].'" class="node" style="font-size: 14px;color: black;text-decoration: none;padding-left: 4px;color:black;">';
		$tree .= ''.$v['name'].'</a>';
		if(isset($v['children'])){
			$tree .= '<ul>';
			foreach($v['children'] as $k1 => $v1 ){
				...
				$tree .= ' <input '.$check1.'style="margin-left:3px;" type="checkbox" name="pri_id" value="'.$v1['id'].'"><a nodeid="'.$v1['id'].'" class="node" style="font-size: 14px;color: black;text-decoration: none;padding-left: 4px;">';
				$tree .= '<a style="color:grey;">'.$v1['name'].'</a>';
