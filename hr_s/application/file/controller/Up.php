...
class Up extends Admin {
	...
	public function file_x($page_info,$user_path,$ts='one'){
		...
		$cf=db()->query("SELECT * FROM sw_file_list where file_name='".$_FILES['file']['name']."' and md5='".$page_info['md5']."' and file_dep_id='".$_SESSION['u_i']['dep_id']."' and bb_new=1 ");//and more_id_bb_no!='1'
	...
	public function up_menu($new='',$vie='up_menu'){
		$id=isset($_GET['id'])?$_GET['id']:'';
		return '<td rowspan="7">
					<div class="widget-body" style="float:left;overflow-y:scroll;height:250px">
						<div class="widget-main padding-8">
							<div id="tree2" class="tree tree-unselectable">
								'.$this->menu_ajax_all("where id='".$_SESSION['u_i']['dep_id']."'","",'','yes','no',$id).'
							</div>
						</div>
					</div>
				</td>';
	...
	public function sql($vie='',$no=''){
		...
		if($vie=='file'){
			...
			if($no=='look'){}else{
				$die_file=isset($_GET['die'])?$_GET['die']:'0';
	...
	public function index_n($vie='file',$no=''){
		//搜索
		$sql_jq=$this->un(addslashes(isset($_GET['sql_jq'])?$_GET['sql_jq']:''));
		$name=$this->un(addslashes(isset($_GET['name'])?$_GET['name']:''));
		$time_star=$this->un(addslashes(isset($_GET['timestar'])?$_GET['timestar']:''));
		if($time_star==''){$time_star='1899-01-01 00:00:00';}
		$time_end=$this->un(addslashes(isset($_GET['timeend'])?$_GET['timeend']:''));
		if($time_end==''){$time='';if($time_star!=='1899-01-01 00:00:00'){$time='>=\''.$time_star.'\'';}}else{$time=" between '".$time_star."' and '".$time_end."'";}
		$size=$this->un(addslashes(isset($_GET['size'])?$_GET['size']:''));
		$hou=$this->un(addslashes(isset($_GET['hou'])?$_GET['hou']:''));
		$xing=$this->un(addslashes(isset($_GET['xing'])?$_GET['xing']:''));
		if($xing){
		...
		$zt=$this->un(addslashes(isset($_GET['zt'])?$_GET['zt']:''));
		$page=$this->un(addslashes(isset($_GET['page'])?$_GET['page']:'1'));
	...
	public function new_menu_go($men_1,$up_yes='',$suv='',$inp_yes='',$no_m='',$idd){
		$menu_ok='';$up_div='';$new_div='';$add_div='';$del_div='';$b_all='';$url='';$inp='';
		foreach ($men_1 as $key=>$m_1){
			if (is_array($m_1)){
				if($no_m){}else{
					...
					$url='onclick="$(\'span[wtq_zs_id=zs]\').html(\''.$m_1['name'].' > \');$(\'input[wtq_up_index_name=xin]\').val(\''.$m_1[$on].'\');event.stopPropagation();ajax_index(\'?xing='.$m_1[$on].'\',this);"';
				}
				if($inp_yes){
					if($m_1['lv']==1){
						$inp='';
					...
					if($up_yes){
						if($m_1['lv']=='1'){
							...
							if($up_yes=='up'){
								...
								$up_onclick='<div title="上传" class="tree-folder-name layerIframe" url="'.url('/file/one_up').'/'.$up_yes.'?id='.$m_1['id'].'" hig="90%" wid="70%">上传</div>';
							}
