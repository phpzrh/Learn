...
class OneUp extends Up {
	...
	public function fenlei_add_all($uo='up'){
		...
		if($lv>5){echo json_encode(array(0,'目录已打最上限'));exit;}
		...
		if($uo=='one_up/change_ajax'){
			$up_onclick='<div title="上传" class="tree-folder-name layerIframe" url="'.url().$uo.'?id='.$userId.'" hig="740px" wid="620px" onclick="return '.$uo.'(\''.$userId.'\')" class="tree-folder-name">上传</div>';
		}else{
			$up_onclick='<div title="上传" class="tree-folder-name layerIframe" url="'.url().$uo.'?id='.$userId.'" hig="90%" wid="70%">上传</div>';
		}
		...
		$div='<div class="tree-folder" wtq_del_id="'.$userId.'">
								<div class="tree-folder-header" onclick="var a=$(this).next(\'div\');if(a.attr(\'op\')==\'close\'){a.css(\'display\',\'\');a.attr(\'op\',\'open\');$(this).children(\'i\').attr(\'class\',\'icon-folder-open\');}else{a.css(\'display\',\'none\');a.attr(\'op\',\'close\');$(this).children(\'i\').attr(\'class\',\'icon-folder-close\');}">
	...
	public function del_fenlei(){
		...
		$files=db()->query("select id from sw_file_list where fenlei in(".$idd.")");
		if(!$files){
	...
	public function file_up(){
		...
		if(Cache::get('more_id')){
			...
			if(isset($this_he)){
				...
				$page_more=array('parents_id'=>$parents_id,'this_bb'=>$page_info['bb'],'qx'=>$page_info['qx'],'import_text'=>$page_info['import_text'],'down'=>$page_info['down'],'bb'=>$page_info['bb'],'file_sta'=>$page_info['file_sta'],'die_time'=>$page_info['die_time'],'fenlei'=>$page_info['fenlei']);
	...
	public function change(){
		...
		if($_FILES['file']['name']){
			//判断是否更新版本
			if($id){
				$page_info=array('down'=>$xz,'fenlei'=>$old['fenlei'],'import_text'=>$gj,'file_sta'=>$sta,'qx'=>$qx_id,'bb'=>$bb);
				if($old['bb_id']=='0'){
					$page_info['bb_id']=$id;
				}else{
				...
				$page_info=array('down'=>$xz,'fenlei'=>$fenlei,'import_text'=>$gj,'file_sta'=>$sta,'qx'=>$qx_id,'bb'=>$bb);
				$page_info['bb_id']='0';
				$page_info['more_id']='0';
			}
