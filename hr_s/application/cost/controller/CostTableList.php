...
//			$email = '1';	//发送邮件
		...
		//修改数据
		db('cost_cost')->where('id',$_POST['id'])->update($data);//修改数据
		$flow = Db::name('cost_cost')->where('id',$_POST['id'])->value('flow');
...
//		//发送邮件给上一个te
//		$user_id_temp = Db::name('cost_log')->field('user_id')->where('data_id',$id)->order('id desc')->select();
//
//		//查找costtable中te编辑功能的人员
//		$te_user_id = Db::name('cost_user_type')->field('user_id')->where('power',4)->select();
